...
ParametricSurface::ParametricSurface(unsigned int nSlices, unsigned int nStacks,
                                     unsigned int sMax, unsigned int tMax)
    : GridMesh(nSlices, nStacks)
{
    _sMax = sMax;
    _tMax = tMax;
}
...
QVector3D ParametricSurface::normalAtParameter(const float& u, const float& v)
{
    ...
    if (t1.isNull())
    {
        ...
        QVector3D uVec2(uDir.getX(), uDir.getY(), uDir.getZ());
        uVec2 = -uVec2;
        t1 = uVec2 - oVec;
    }
    if (t2.isNull())
    {
        ...
        QVector3D vVec2(vDir.getX(), vDir.getY(), vDir.getZ());
        vVec2 = -vVec2;
        t2 = vVec2 - oVec;
    }
