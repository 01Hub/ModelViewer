...
void UVGenerationDialog::savePreset(const QString& presetName)
{
    if (presetName.isEmpty())
        return;

    QSettings settings(QCoreApplication::organizationName(), QCoreApplication::applicationName());
    settings.beginGroup("UVGenerationPresets");
    settings.beginGroup(presetName);

    // Save method
    settings.setValue("method", static_cast<int>(getSelectedMethod()));

    // Get current config and save all values
    UVConfig config = getUVConfig();
...
void UVGenerationDialog::loadPreset(const QString& presetName)
{
    if (presetName.isEmpty())
        return;
    ...
    // Load method
    UVMethod method = static_cast<UVMethod>(settings.value("method", 0).toInt());
    setMethod(method);
...
QStringList UVGenerationDialog::getAvailablePresets() const
{
    QSettings settings(QCoreApplication::organizationName(), QCoreApplication::applicationName());
    settings.beginGroup("UVGenerationPresets");
...
void UVGenerationDialog::deletePreset(const QString& presetName)
{
    if (presetName.isEmpty())
        return;
